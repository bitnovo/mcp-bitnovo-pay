...
import { validateCreatePaymentOnchain, validateCreatePaymentRedirect, validateGetPaymentStatus, validateAmountForCurrency, } from '../utils/validation.js';
...
export class PaymentService {
    ...
    async createOnchainPayment(input) {
        ...
        if (!validation.success) {
            ...
            throw new PaymentServiceError(`Invalid input: ${validation.error.message}`, 400, validation.error.code, { field: validation.error.field });
        }
        ...
        try {
            ...
            const requestedCurrency = currencies.find(c => c.symbol === validInput.input_currency);
            if (!requestedCurrency) {
                throw new PaymentServiceError(`Currency ${validInput.input_currency} is not supported`, 400, 'INVALID_CURRENCY', { requestedCurrency: validInput.input_currency });
            }
            if (!requestedCurrency.isActive) {
                throw new PaymentServiceError(`Currency ${validInput.input_currency} is currently not available`, 400, 'CURRENCY_INACTIVE', { currency: validInput.input_currency });
            }
            // Validate amount against currency limits
            const amountValidation = validateAmountForCurrency(validInput.amount_eur, requestedCurrency);
            if (!amountValidation.success) {
                throw new PaymentServiceError(amountValidation.error.message, 400, amountValidation.error.code, {
            ...
            logger.error('Failed to create onchain payment', error, {
            ...
            throw new PaymentServiceError('Internal error creating onchain payment', 500, 'INTERNAL_ERROR', { originalError: error });
        }
    }
    /**
    ...
    async createRedirectPayment(input) {
        ...
        try {
            ...
            logger.error('Failed to create redirect payment', error, {
            ...
            throw new PaymentServiceError('Internal error creating redirect payment', 500, 'INTERNAL_ERROR', { originalError: error });
        }
    }
    /**
    ...
    async getPaymentDetails(identifier) {
        ...
        if (!identifier ||
            ...
            throw new PaymentServiceError('Invalid payment identifier format', 400, 'INVALID_IDENTIFIER');
        }
        try {
            ...
            throw new PaymentServiceError('Internal error getting payment details', 500, 'INTERNAL_ERROR', { originalError: error, paymentId: identifier });
        }
    }
    /**
    ...
    async getPaymentStatus(input) {
        ...
        try {
            ...
            const paymentStatus = await this.apiClient.getPaymentStatus(validInput.identifier);
            ...
            logger.info('Payment status retrieved successfully', {
                ...
                hasAmounts: !!(enhancedStatus.confirmedAmount || enhancedStatus.unconfirmedAmount),
            ...
            throw new PaymentServiceError('Internal error getting payment status', 500, 'INTERNAL_ERROR', { originalError: error, paymentId: validInput.identifier });
        }
    }
    /**
     * Validate redirect URLs are properly formatted
     */
    validateRedirectUrls(urlOk, urlKo) {
        const errors = [];
        ...
        if (errors.length > 0) {
            throw new PaymentServiceError(`Invalid redirect URLs: ${errors.join(', ')}`, 400, 'INVALID_REDIRECT_URLS', { errors });
        }
    }
    /**
     * Enhance payment status with additional business logic and user-friendly information
     */
    enhancePaymentStatus(status) {
        ...
        if (status.status === 'IA' && status.cryptoAmount) {
            const totalReceived = (status.confirmedAmount || 0) + (status.unconfirmedAmount || 0);
     ...
     */
    isPaymentExpired(expirationTime) {
        try {
