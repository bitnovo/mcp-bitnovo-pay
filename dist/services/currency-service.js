...
export class CurrencyService {
     ...
     */
    async getCurrenciesCatalog(input) {
        ...
        if (!validation.success) {
            ...
            throw new CurrencyServiceError(`Invalid input: ${validation.error.message}`, 400, validation.error.code, { field: validation.error.field });
        }
        ...
        try {
            ...
            const filteredCurrencies = this.applyCurrencyFilters(allCurrencies, filters);
            ...
            logger.error('Failed to get currencies catalog', error, {
            ...
            throw new CurrencyServiceError('Internal error getting currencies catalog', 500, 'INTERNAL_ERROR', { originalError: error });
        }
    }
    /**
    ...
    async getCurrencies() {
        ...
        try {
            ...
            const enhancedCurrencies = currencies.map(currency => this.enhanceCurrency(currency));
            // Update cache
            ...
            logger.error('Failed to fetch currencies from API', error, {
     ...
     */
    async isCurrencyValidForAmount(symbol, amount) {
     ...
     */
    async getCurrenciesForAmount(amount) {
        const allCurrencies = await this.getCurrencies();
     ...
     */
    applyCurrencyFilters(currencies, filters) {
        ...
        if (filters.filterByAmount !== undefined) {
            ...
            filtered = filtered.filter(c => {
                if (amount < c.minAmount)
                    return false;
                if (c.maxAmount !== null && amount > c.maxAmount)
                    return false;
                return true;
            });
        }
        ...
        if (filters.maxAmount !== undefined) {
            filtered = filtered.filter(c => c.maxAmount !== null && c.maxAmount <= filters.maxAmount);
        }
        // Filter by blockchain
        if (filters.blockchain) {
            filtered = filtered.filter(c => c.blockchain.toLowerCase() === filters.blockchain.toLowerCase());
        }
