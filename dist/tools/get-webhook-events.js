...
export const getWebhookEventsTool = {
    ...
    description: 'Retrieve webhook events received from Bitnovo Pay. ' +
    ...
    inputSchema: {
        ...
        properties: {
            identifier: {
                type: 'string',
                description: 'Optional: Filter events by payment identifier (UUID). ' +
...
export class GetWebhookEventsHandler {
    ...
    async handle(args) {
        ...
        try {
            // Validate input
            const input = GetWebhookEventsInputSchema.parse(args);
            ...
            if (error instanceof Error && error.message.includes('not initialized')) {
                ...
                throw new Error('Webhook event store not initialized. Ensure webhooks are enabled (WEBHOOK_ENABLED=true).');
            }
            logger.error('Error retrieving webhook events', error, {
                duration,
                operation: 'get_webhook_events_error',
            });
            throw new Error(`Failed to retrieve webhook events: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
}
/**
 ...
 */
export function getWebhookEventsHandler() {
    return new GetWebhookEventsHandler();
}
//# sourceMappingURL=get-webhook-events.js.map