    ...
    inputSchema: {
        ...
        properties: {
            filter_by_amount: {
                type: 'number',
                minimum: 0.01,
                description: 'Optional EUR amount to filter currencies that support this payment amount. Use this if user asks "which cryptos accept 50 euros?" or similar.',
            },
        },
...
export class ListCurrenciesCatalogHandler {
    ...
    async handle(args) {
        ...
        try {
            ...
            const duration = Date.now() - startTime;
            logger.error('list_currencies_catalog failed', error, {
...
export function listCurrenciesCatalogHandler(currencyService) {
    return new ListCurrenciesCatalogHandler(currencyService);
}
//# sourceMappingURL=list-currencies-catalog.js.map