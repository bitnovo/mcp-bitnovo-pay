 ...
 */
export const getWebhookUrlTool = {
...
Optional validation check ensures the URL is publicly accessible.`,
    inputSchema: {
        ...
        properties: {
            validate: {
                type: 'boolean',
                description: 'If true, performs a health check on the URL to verify it is publicly accessible (default: false)',
                default: false,
            },
        },
    },
    ...
    async handle(args) {
        ...
        try {
            ...
            if (!this.webhookServer) {
                throw new Error('Webhooks are not enabled. Set WEBHOOK_ENABLED=true to use webhooks.');
            }
            // Get webhook URL
            const webhookUrl = this.webhookServer.getWebhookUrl();
            if (!webhookUrl) {
                throw new Error('Webhook URL not available. Tunnel may not be started or configured properly.');
            }
            // Get tunnel info
...
Note: This ngrok URL is persistent and will not change between restarts${tunnelInfo?.['ngrokDomain']
                        ? ' (using custom domain)'
...
Note: This is your server's public URL. Ensure your firewall allows inbound HTTPS traffic.`;
            ...
            const duration = Date.now() - startTime;
            logger.error('get_webhook_url failed', error instanceof Error ? error : new Error(String(error)), {
                duration,
 ...
 */
export function getWebhookUrlHandler(webhookServer) {
    return new GetWebhookUrlHandler(webhookServer);
}
//# sourceMappingURL=get-webhook-url.js.map