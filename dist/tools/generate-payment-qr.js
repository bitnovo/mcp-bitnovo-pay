...
import { generateGatewayUrlFromWebUrl, } from '../utils/url-utils.js';
import { getQrCache } from '../utils/qr-cache.js';
const logger = getLogger();
export const generatePaymentQrTool = {
    ...
    inputSchema: {
        ...
        properties: {
            ...
            qr_type: {
                ...
                default: 'both',
                description: 'Type of QR to generate: "address": Only crypto address (customer must enter amount manually), "payment_uri": Address + amount included (recommended for specific amounts), "both": Generate both types (recommended), "gateway_url": QR of the payment gateway URL (only for redirect payments)',
            },
            size: {
                type: 'integer',
                minimum: 100,
                maximum: 2000,
                default: 512,
                description: 'QR code size in pixels. Recommended: 512 for mobile/desktop, 800-1200 for print, 1600-2000 for high-quality print',
            },
            style: {
                type: 'string',
                enum: ['basic', 'branded'],
                default: 'branded',
                description: 'QR code style: "basic": Plain black and white QR, "branded": Includes cryptocurrency logo in center (more professional)',
            },
            branding: {
                type: 'boolean',
                default: true,
            ...
            gateway_environment: {
                ...
                description: 'Environment for gateway URL generation (auto-detected if not specified)',
            },
        },
        required: ['identifier'],
...
export class GeneratePaymentQrHandler {
    ...
    async handle(args) {
        ...
        try {
            ...
            const { identifier, qr_type = 'both', size = 300, style = 'branded', branding = true, gateway_environment, } = validation.data;
            ...
            const paymentData = await this.paymentService.getPaymentDetails(identifier);
            ...
            if ((qr_type === 'address' || qr_type === 'both') &&
                paymentData.address) {
                const cachedAddress = cache.get(identifier, 'address', size, style, branding);
                if (cachedAddress) {
                    response.qr_address = cachedAddress;
                }
                else {
                    const addressQr = await generateOptimizedQrCode(paymentData.address, qrOptions);
                    response.qr_address = {
                    ...
                    cache.set(identifier, 'address', size, style, branding, response.qr_address);
                }
            ...
            if ((qr_type === 'payment_uri' || qr_type === 'both') &&
                paymentData.paymentUri) {
                const cachedPaymentUri = cache.get(identifier, 'payment_uri', size, style, branding);
                if (cachedPaymentUri) {
                    response.qr_payment_uri = cachedPaymentUri;
                }
                else {
                    const paymentUriQr = await generateOptimizedQrCode(paymentData.paymentUri, qrOptions);
                    ...
                    cache.set(identifier, 'payment_uri', size, style, branding, response.qr_payment_uri);
                }
            ...
            if (qr_type === 'gateway_url') {
                ...
                if (!paymentData.webUrl) {
                    throw new Error('Gateway URL QR generation requires a redirect payment with web_url');
                }
                const gatewayUrl = generateGatewayUrlFromWebUrl(paymentData.webUrl, gateway_environment);
                if (!gatewayUrl) {
                    throw new Error('Failed to generate gateway URL from payment web_url');
                }
                const cachedGatewayUrl = cache.get(identifier, 'gateway_url', size, style, branding);
                if (cachedGatewayUrl) {
                    response.qr_gateway_url = cachedGatewayUrl;
                }
                else {
                    const gatewayQrOptions = {
                        size,
                        includeBranding: branding,
                        style,
                    ...
                    const gatewayUrlQr = await generateOptimizedQrCode(gatewayUrl, gatewayQrOptions);
                    ...
                    cache.set(identifier, 'gateway_url', size, style, branding, response.qr_gateway_url);
                }
            ...
            if (!response.qr_address &&
                !response.qr_payment_uri &&
                !response.qr_gateway_url) {
                throw new Error('No QR codes could be generated - payment may not have required data (address, payment_uri, or web_url)');
            }
            ...
            const duration = Date.now() - startTime;
            logger.error('generate_payment_qr failed', error, {
    ...
    validateInput(args) {
        ...
        if (input.qr_type &&
            ...
            return {
                success: false,
                error: 'qr_type must be one of: address, payment_uri, both, gateway_url',
            };
        }
        // Validate size
        ...
        if (input.gateway_environment &&
            !['development', 'testing', 'production'].includes(input.gateway_environment)) {
            return {
                success: false,
                error: 'gateway_environment must be one of: development, testing, production',
            };
        }
...
export function generatePaymentQrHandler(paymentService, currencyService) {
    return new GeneratePaymentQrHandler(paymentService, currencyService);
}
//# sourceMappingURL=generate-payment-qr.js.map