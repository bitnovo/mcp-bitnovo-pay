...
export const getPaymentStatusTool = {
    ...
    description: 'Query the current status and details of a payment by its unique identifier. USE THIS WHEN: User wants to check if a payment was completed, or get details about a previously created payment. RESULT: Payment status (PE=Pending, CO=Completed, EX=Expired, CA=Cancelled), amounts, and confirmation info. Status descriptions: PE (Pending): Waiting for customer to pay, CO (Completed): Payment confirmed on blockchain, EX (Expired): Payment time limit exceeded, CA (Cancelled): Payment was cancelled. EXAMPLES: "Check payment status", "Is payment abc-123 completed?", "Status of my payment". Note: Exchange rate is not included in response as it may not be accurate for non-EUR fiat currencies.',
    inputSchema: {
        type: 'object',
        properties: {
            identifier: {
...
export class GetPaymentStatusHandler {
    ...
    async handle(args) {
        ...
        try {
            ...
            logger.info('get_payment_status completed successfully', {
                ...
                status: paymentStatus.status,
                hasAmounts: !!(paymentStatus.confirmedAmount || paymentStatus.unconfirmedAmount),
            ...
            const duration = Date.now() - startTime;
            logger.error('get_payment_status failed', error, {
     ...
     */
    getStatusGuidance(status, paymentStatus) {
        const guidanceMap = {
     ...
     */
    calculateProgress(paymentStatus) {
     ...
     */
    shouldNotify(status, previousStatus) {
     ...
     */
    maskSensitiveData(paymentStatus) {
        return {
            identifier: paymentStatus.identifier,
            status: paymentStatus.status,
...
export function getPaymentStatusHandler(paymentService) {
