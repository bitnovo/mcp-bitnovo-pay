...
const currencySymbolSchema = z
    .string()
    .regex(/^[A-Z0-9_]+$/, 'Currency symbol must be uppercase alphanumeric with underscores');
const fiatCurrencySchema = z
    .string()
    .regex(/^[A-Z]{3}$/, 'Fiat currency must be 3-letter uppercase code (e.g., EUR, USD)');
...
export function validateCreatePaymentOnchain(input) {
    return validateInput(createPaymentOnchainSchema, input);
}
export function validateCreatePaymentRedirect(input) {
    return validateInput(createPaymentRedirectSchema, input);
}
export function validateGetPaymentStatus(input) {
    return validateInput(getPaymentStatusSchema, input);
}
export function validateListCurrenciesCatalog(input) {
    return validateInput(listCurrenciesCatalogSchema, input);
}
// Additional business logic validators
export function validateAmountForCurrency(amount, currency) {
...
export const envConfigSchema = z.object({
    ...
    BITNOVO_BASE_URL: z.string().url('Base URL must be valid HTTP/HTTPS URL'),
    BITNOVO_DEVICE_SECRET: z.string().optional(),
    LOG_LEVEL: z
    ...
    WEBHOOK_ENABLED: z.preprocess(val => (val === 'true' || val === '1'), z.boolean().default(false)),
    ...
    TUNNEL_ENABLED: z.preprocess(val => (val === 'true' || val === '1'), z.boolean().default(true)),
    TUNNEL_PROVIDER: z
...
export function getHttpStatusForValidationError(code) {
    const statusMap = {
        INVALID_UUID: 400,
