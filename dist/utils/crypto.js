 ...
 */
export function validateWebhookSignature(deviceSecret, nonce, rawBody, receivedSignature) {
    try {
        // Input validation
        ...
        logger.error('HMAC validation error', error instanceof Error ? error : new Error(String(error)), {
 ...
 */
export function createTestSignature(deviceSecret, nonce, body) {
 ...
 */
export function validateNonce(nonce, maxAge = 5 * 60 * 1000, // 5 minutes default
seenNonces = new Set()) {
...
let globalNonceCache = null;
export function getNonceCache() {
    if (!globalNonceCache) {
