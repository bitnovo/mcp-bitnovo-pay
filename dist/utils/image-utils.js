 ...
 */
async function initializeBitnovoLogo() {
    try {
        // Resolve path to BitnovoPay.svg asset
        // When running from src/ (dev): ../assets/BitnovoPay.svg
        // When running from dist/ (prod): ../assets/BitnovoPay.svg
        const logoPath = join(__dirname, '..', 'assets', 'BitnovoPay.svg');
        ...
        <svg width="100" height="100" xmlns="http://www.w3.org/2000/svg">
          ...
          <text x="50" y="60" font-family="Arial, sans-serif" font-size="28" font-weight="bold"
            ...
            logger.error('Failed to initialize fallback logo', fallbackError, {
 ...
 */
async function generateUltraFastQR(data, size = 300) {
    const startTime = process.hrtime.bigint();
    try {
        ...
        logger.error('Ultra-fast QR generation failed', error, {
 ...
 */
async function addLogoOverlay(qrBuffer, logoBuffer, size) {
    const startTime = process.hrtime.bigint();
    try {
        ...
        const circleBackground = Buffer.from(`<svg width="${backgroundSize}" height="${backgroundSize}">
        ...
        logger.error('Logo overlay failed', error, {
...
export async function generateOptimizedQrCode(data, options) {
    ...
    try {
        ...
        if (options.isGatewayUrl) {
            ...
            logoBuffer = await downloadCryptoLogo(options.currencyImageUrl, options.currencySymbol);
        }
 ...
 */
export async function generateFastQrCode(data, size, errorCorrectionLevel = 'M') {
    return await generateUltraFastQR(data, size);
}
/**
 * Initialize fast image processing system
 */
export async function initializeFastImageProcessing() {
