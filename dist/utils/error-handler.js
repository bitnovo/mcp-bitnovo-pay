...
export class MCPError extends Error {
    ...
    details;
    operation;
    constructor(message, statusCode, code, details, operation) {
        super(message);
        this.name = 'MCPError';
...
export class ErrorHandler {
     ...
     */
    static processError(error, operation) {
        const timestamp = new Date().toISOString();
        // Handle known MCP errors
        if (error instanceof MCPError) {
            logger.error(`MCP Error in ${operation || error.operation || 'unknown'}`, error, {
                code: error.code,
     ...
     */
    static formatMCPError(error, operation) {
     ...
     */
    static sanitizeErrorForClient(errorDetails) {
...
export function createTimeoutError(operation, timeout) {
    return new TimeoutError(`Operation '${operation}' timed out after ${timeout}ms`, { operation, timeout });
}
export function createServiceUnavailableError(service, reason) {
...
export const { processError, formatMCPError, isRetryableError, getStatusCode } = ErrorHandler;
