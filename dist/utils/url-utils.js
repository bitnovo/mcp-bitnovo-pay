 ...
 */
export const DEFAULT_GATEWAY_DOMAINS = {
 ...
 */
export function extractShortIdentifierFromUrl(webUrl) {
    try {
        ...
        logger.error('Failed to parse web URL for short identifier extraction', error, {
            webUrl,
 ...
 */
export function determineEnvironment(baseUrl) {
 ...
 */
export function generateGatewayUrl(shortIdentifier, environment, customDomains) {
 ...
 */
export function generateGatewayUrlFromWebUrl(webUrl, environment, customDomains) {
 ...
 */
export function isValidGatewayUrl(url) {
    try {
        const urlObj = new URL(url);
        // Check if domain matches known Bitnovo domains
        const isKnownDomain = Object.values(DEFAULT_GATEWAY_DOMAINS).some(domain => urlObj.hostname.includes(domain.replace('https://', '')));
        if (!isKnownDomain) {
            return false;
        }
        ...
        return (/^[a-zA-Z0-9]+$/.test(shortIdentifier) &&
            shortIdentifier.length >= 4 &&
            shortIdentifier.length <= 20);
    }
    catch {
 ...
 */
export function getCurrentEnvironmentDomain(customDomains) {
    const baseUrl = process.env.BITNOVO_BASE_URL || '';
 ...
 */
export function createDisplayUrl(gatewayUrl) {
    try {
