...
import { initializeEventStore, shutdownEventStore, } from './storage/webhook-events.js';
...
// MCP Tools
import { createPaymentOnchainTool, createPaymentOnchainHandler, } from './tools/create-payment-onchain.js';
import { createPaymentLinkTool, createPaymentLinkHandler, } from './tools/create-payment-link.js';
import { getPaymentStatusTool, getPaymentStatusHandler, } from './tools/get-payment-status.js';
import { listCurrenciesCatalogTool, listCurrenciesCatalogHandler, } from './tools/list-currencies-catalog.js';
import { generatePaymentQrTool, generatePaymentQrHandler, } from './tools/generate-payment-qr.js';
import { getWebhookEventsTool, getWebhookEventsHandler, } from './tools/get-webhook-events.js';
import { getWebhookUrlTool, getWebhookUrlHandler, } from './tools/get-webhook-url.js';
import { getTunnelStatusTool, getTunnelStatusHandler, } from './tools/get-tunnel-status.js';
...
class MCPBitnovoServer {
     ...
     */
    initializeWebhookInfrastructure() {
            ...
            try {
                switch (name) {
                    case 'create_payment_onchain': {
                        const handler = createPaymentOnchainHandler(this.paymentService, this.currencyService);
                        const result = await handler.handle(args);
                        const contentBlocks = createMcpContentBlocks(result, 'create_payment_onchain');
                        return {
                    ...
                    case 'create_payment_link': {
                        ...
                        const result = await handler.handle(args);
                        const contentBlocks = createMcpContentBlocks(result, 'create_payment_link');
                        return {
                            content: contentBlocks,
                        };
                    }
                    case 'get_payment_status': {
                        ...
                        const result = await handler.handle(args);
                        const contentBlocks = createMcpContentBlocks(result, 'get_payment_status');
                        return {
                            content: contentBlocks,
                        };
                    }
                    case 'list_currencies_catalog': {
                        ...
                        const result = await handler.handle(args);
                        const contentBlocks = createMcpContentBlocks(result, 'list_currencies_catalog');
                        return {
                            content: contentBlocks,
                        };
                    }
                    case 'generate_payment_qr': {
                        const handler = generatePaymentQrHandler(this.paymentService, this.currencyService);
                        const result = await handler.handle(args);
                        const contentBlocks = createMcpContentBlocks(result, 'generate_payment_qr');
                        return {
                            content: contentBlocks,
                        };
                    }
                    case 'get_webhook_events': {
                        if (!this.config.webhookEnabled) {
                            throw new Error('Webhooks are not enabled. Set WEBHOOK_ENABLED=true to use this tool.');
                        }
                        const handler = getWebhookEventsHandler();
                        const result = await handler.handle(args);
                        const contentBlocks = createMcpContentBlocks(result, 'get_webhook_events');
                        return {
                            content: contentBlocks,
                        };
                    }
                    case 'get_webhook_url': {
                        ...
                        const result = await handler.handle(args || {});
                        const contentBlocks = createMcpContentBlocks(result, 'get_webhook_url');
                        return {
                            content: contentBlocks,
                        };
                    }
                    case 'get_tunnel_status': {
                        ...
                        const contentBlocks = createMcpContentBlocks(result, 'get_tunnel_status');
                        return {
                            content: contentBlocks,
                        };
                    }
        ...
        try {
            ...
            if (this.webhookServer) {
                ...
                try {
                    ...
                    logger.warn('Continuing without webhook server - MCP tools will still work', {
