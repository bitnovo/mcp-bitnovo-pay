  ...
  async getPaymentStatus(identifier: string): Promise<PaymentStatus> {
    try {
      ...
      const response = await this.httpClient.get<
        BitnovoApiResponse<BitnovoPaymentInfoResponse>
      ...
      let paymentInfo: BitnovoPaymentInfoResponse;
      ...
      if (response.data?.data) {
        ...
        paymentInfo = response.data[0] as BitnovoPaymentInfoResponse;
      } else if (
        response.data &&
        ...
        paymentInfo = response.data as BitnovoPaymentInfoResponse;
      } else {
        // Unexpected format
        logger.error(
