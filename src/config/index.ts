...
const requiredEnvVars = ['BITNOVO_DEVICE_ID', 'BITNOVO_BASE_URL'] as const;
...
function validateDeviceId(deviceId: string): void {
  if (deviceId.length < 8) {
    throw new ConfigurationError(
      'BITNOVO_DEVICE_ID must be at least 8 characters'
    );
  }
...
function validateBaseUrl(baseUrl: string): void {
  try {
    ...
    throw new ConfigurationError(
      `BITNOVO_BASE_URL must be a valid HTTP/HTTPS URL, got: ${baseUrl}`
    );
  }
}
