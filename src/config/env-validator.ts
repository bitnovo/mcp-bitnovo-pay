  ...
  private static validateBusinessRules(
    ...
    if (config.BITNOVO_DEVICE_SECRET) {
      if (config.BITNOVO_DEVICE_SECRET.length < 16) {
        warnings.push(
          'BITNOVO_DEVICE_SECRET should be at least 16 characters for security'
        );
      }
    } else {
      warnings.push(
        'BITNOVO_DEVICE_SECRET not set - webhook validation will be disabled'
      );
    }
    ...
    if (config.NODE_ENV === 'production') {
      if (!config.BITNOVO_DEVICE_SECRET) {
        warnings.push(
          'Consider setting BITNOVO_DEVICE_SECRET in production for webhook security'
        );
      }
